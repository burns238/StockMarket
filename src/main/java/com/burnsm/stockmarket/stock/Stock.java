...
public abstract class Stock {
    ...
    private List<Trade> getTradesFromTheLastFifteenMinutes() {
        ...
        return this.getTrades().stream().filter(recentTrades).collect(Collectors.toList());
    }
    ...
    public double calculateVolumeWeightedStockPrice() {
        ...
        double priceByQuantity = recentTrades.stream().map(t -> t.getTradedPrice() * t.getQuantity()).reduce(0.0,
                (subtotal, v) -> subtotal + v);

        double quantity = recentTrades.stream().mapToInt(t -> t.getQuantity()).sum();
